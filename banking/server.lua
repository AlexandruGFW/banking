...
AddEventHandler("chatMessage", function(s, n, m)
  ...
  if command[1] == "/checkbalance" then
    TriggerClientEvent("chatMessage", s, "BANK", {0, 200, 0}, "Balance ta curenta: ^3$" .. bankbalance)
    TriggerClientEvent("banking:updateBalance", source, bankbalance)
    CancelEvent()
  end
  ...
  if command[1] == "/transfer" then
    if(command[2] ~= nil and tonumber(command[3]) > 0) then
      ...
      TriggerClientEvent('chatMessage', s, "BANK", {0, 200, 0}, "^1Foloseste comanda /transfer [id] [amount]^0")
    end
    CancelEvent()
  end
end)
  ...
  if not amount then return end
    ...
    if(string.len(rounded) >= 9) then
      ...
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        vRPclient.notify(user_id, {"~r~Nu ai destuli bani in cont!"})
      end
    end
end)
...
AddEventHandler('bank:transfer', function(fromPlayer, toPlayer, amount)
  ...
  if tonumber(fPlayer) == tonumber(toPlayer) then
    TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Nu iti poti transfera singur bani!")
    CancelEvent()
  else
    local rounded = round(tonumber(amount), 0)
    if(string.len(rounded) >= 9) then
      TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Numarul este prea mare!")
      CancelEvent()
    else
      local bankbalance = vRP.getBankMoney({user_id})
      if(tonumber(rounded) <= tonumber(bankbalance)) then
        ...
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Transferat: ^1$"..math.floor(rounded))
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^0Balanta ta noua: ^4$" .. math.floor((bankbalance - rounded)))
        ...
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Ap primit: ^2$"..math.floor(rounded))
        TriggerClientEvent('chatMessage', toPlayer, "", {0, 255, 0}, "^0Balanta noua: ^4$" .. math.floor((bankbalance2 + rounded)))
        CancelEvent()
      else
        TriggerClientEvent('chatMessage', fPlayer, "", {0, 255, 0}, "^1Nu ai destuli bani in cont!")
      end
    end    
  end
end)
