    ...
    while true do
      ...
      if(IsNearBank() or IsNearATM()) then
        if (atBank == false) then
          TriggerEvent('chatMessage', "", {255, 255, 255}, "^2Apasa ^0E^2 pentru a accesa banca");
        end
        atBank = true
        if IsControlJustPressed(1, 51)  then -- IF INPUT_PICKUP Is pressed
          if (IsInVehicle()) then
            TriggerEvent('chatMessage', "", {255, 0, 0}, "^1Nu poti folosi atm-ul cand esti in vehicul!");
          else
            if bankOpen then
              closeGui()
              bankOpen = false
...
AddEventHandler('bank:deposit', function(amount)
  if(IsNearBank() == true or depositAtATM == true and IsNearATM() == true or depositAnywhere == true ) then
    if (IsInVehicle()) then
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Nu poti folosi atm-ul cand esti in vehicul!");
    else
      TriggerServerEvent("bank:deposit", tonumber(amount))
      TriggerEvent('chatMessage', "BANK", {0, 200, 0}, "^0Depozitul tau ^5$"..amount);
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Numai tu poti depozita in banca!");
  end
end)
...
AddEventHandler('bank:givecash', function(toPlayer, amount)
  if(IsNearPlayer(toPlayer) == true or giveCashAnywhere == true) then
    ...
    if (playing ~= false) then
      ...
      TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Acest jucator nu este online!");
    end
  else
    TriggerEvent('chatMessage', "", {0, 255, 0}, "^1Tu nu esti langa jucator!");
  end
end)
